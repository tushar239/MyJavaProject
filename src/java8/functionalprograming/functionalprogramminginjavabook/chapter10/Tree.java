...
public abstract class Tree<A extends Comparable<A>> { // The Tree class is parameterized and the parameter type must extend Comparable.
    ...
    // pg 291
    public static <A extends Comparable<A>> boolean member(Tree<A> root, A value) {
    ...
    public static <A extends Comparable<A>, B extends Comparable<B>> Tree<B> map(Tree<A> tree, Function<A, B> f) {
        ...
        return foldLeft_PreOrder_Using_One_Function(
                tree,
                identity,
                treeValue -> leftSubTreeResult -> rightSubTreeResult -> new DefaultTree<B>(f.apply(treeValue), leftSubTreeResult, rightSubTreeResult)
        );
    }
...
*/
        ...
        System.out.println("map function...");
        {
            ...
            Tree<Integer> mappedTree = map(tree, a -> a * 2);
            System.out.println(foldLeft_PreOrder_Using_One_Function(
                    mappedTree,
                    "",
                    treeValue -> leftSubTreeResult -> rightSubTreeResult -> treeValue + "," + leftSubTreeResult + rightSubTreeResult)
            ); // 22,16,10,6,12,20,28,26,30,
