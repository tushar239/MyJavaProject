...
public class _4LongestCommonSubSequence {
    ...
    public static void main(String[] args) {
        {
            ...
            System.out.println();
            int lca = LCA_Brute_Force(s1.toCharArray(), s2.toCharArray(), 0, s1.length() - 1, 0, s2.length() - 1);
            System.out.println();
            System.out.println("Max common subsequence: " + lca);//3
            ...
            System.out.println();
            lca = LCA_Top_Down_Approach(s1.toCharArray(), s2.toCharArray(), 0, s1.length() - 1, 0, s2.length() - 1, new HashMap<>());
            System.out.println();
            System.out.println("Max common subsequence: " + lca);//3
        }
        System.out.println();
    ...
    private static int LCA_Brute_Force(char[] S1, char[] S2, int s1Start, int s1End, int s2Start, int s2End) {
        cnt++;
        ...
        if (s1Char == s2Char) {
            ...
            return 1 + LCA_Brute_Force(S1, S2, s1Start, s1End - 1, s2Start, s2End - 1);
        }

        // otherwise, get the max of (reducing first string's end index by 1, reducing second string's end index by 1)
        return Math.max(LCA_Brute_Force(S1, S2, s1Start, s1End - 1, s2Start, s2End),
                LCA_Brute_Force(S1, S2, s1Start, s1End, s2Start, s2End - 1));
    }

    private static int count = 0;

    private static int LCA_Top_Down_Approach(char[] S1, char[] S2, int s1Start, int s1End, int s2Start, int s2End, Map<String, Integer> memo) {
        count++;
        ...
        if (s1Char == s2Char) {
            System.out.println("    " + s1Char + " is a part of a common subsequence");// same character will be printed many times because LCA function will be called many times with the same parameters. So, use Dynamic Programming.
            int resultFromRemaining = LCA_Top_Down_Approach(S1, S2, s1Start, s1End - 1, s2Start, s2End - 1, memo);
            memo.put((s1End - 1) + ":" + (s2End - 1), resultFromRemaining);
            return 1 + resultFromRemaining;
        }

        // otherwise, get the max of (reducing first string's end index by 1, reducing second string's end index by 1)
        int resultFromRemainingS1 = LCA_Top_Down_Approach(S1, S2, s1Start, s1End - 1, s2Start, s2End, memo);
        memo.put((s1End - 1) + ":" + s2End, resultFromRemainingS1);

        int resultFromRemainingS2 = LCA_Top_Down_Approach(S1, S2, s1Start, s1End, s2Start, s2End - 1, memo);
