    ...
    private static int findMinAbsDiffBetElementsFromRows(int[][] A, int startRow, int endRow, int startCol, int endCol) {
        ...
        for (int col = startCol; col <= endCol; col++) {
            ...
            if (startRow < endRow) {// for making sure that A[startRow + 1] doesn't throw ArrayIndexOutOfBoundException
                ...
                int elementToCompareWith = A[startRow][col];

                // diff between elementToCompareWith and closestGreaterOrEqualElement from next row
                int closestGreaterOrEqualElementIndexFromNextRow = binarySearchToFindClosestGreaterOrEqualElement(A[startRow + 1], startCol, endCol, elementToCompareWith);

                if(closestGreaterOrEqualElementIndexFromNextRow == -1) closestGreaterOrEqualElementIndexFromNextRow = endCol;
                int closetGreaterOrEqualElementFromNextRow = A[startRow + 1][closestGreaterOrEqualElementIndexFromNextRow];

                int diff1 = Math.abs(elementToCompareWith - closetGreaterOrEqualElementFromNextRow);

                // diff between elementToCompareWith and closestLesserOrEqualElement from next row
                int diff2 = Integer.MAX_VALUE;
                int closetLesserOrEqualElementFromNextRow = -1;
                if (closestGreaterOrEqualElementIndexFromNextRow > 0) {
                    ...
                    closetLesserOrEqualElementFromNextRow = A[startRow + 1][closetLesserOrEqualElementIndexFromNextRow];
                    diff2 = Math.abs(elementToCompareWith - closetLesserOrEqualElementFromNextRow);
                }
